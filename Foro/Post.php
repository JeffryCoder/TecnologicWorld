...
$mysqli = new mysqli("localhost","root","1234","basededatos");
  ...
  $Imagen = "http://localhost/".$persona['FotoPerfil'];
  $Usuario = $persona['Nickname'];
	echo '<div class="Post">
		...
		<div class="PersonaPost">
	  	...
	  	<a href="http://localhost/php/Buscador/Perfil.php?user='.$Usuario.'">
				<h3>'.$Usuario.'</h3>
			</a>
...
<?php $foto = "../".$_SESSION['FotoUsuario'];?>
<div class="FormularioParaComentar">
	<div class="FormularioComentar">
		<?php echo'<img src="'.$foto.'" alt="imagen"/>'?>
		<?php $ID = $_GET['ID'];
	echo'
...
$sql = "SELECT * FROM comentariosdelforo
INNER JOIN usuarios
ON usuarios.ID = comentariosdelforo.ID_Persona
...
for($i = 0;$i < count($Comentario); $i++){
	...
	echo '<div class="Comentario">';
		echo '<div class="UserQueComenta">';
				...
				echo '<a href="http://localhost/php/Buscador/Perfil.php?user='.$Comentario[$i]['Nickname'].'" target="_blank"><h4>'.$Comentario[$i]['Nickname'].'</h4></a>';
		echo '</div>';
